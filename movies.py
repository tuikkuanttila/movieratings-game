...
def play(filename):
    ...
    while(char != 'q'):
        ...
        if(char == 'h'):
            ...
            print('Ratings are fetched from the OMDb API.\n')
            print('Movies are stored in movielist.txt and they are shuffled randomly each time.\n')
            print('You can add more movies to the file, put each on their own line.\n')
        ...
        if(char == 'y'):
            if (index < end - 2):
                ...
                answer = raw_input('Do you think *%s* has a higher rating than *%s*? (y or n) ' %(title1,title2))
